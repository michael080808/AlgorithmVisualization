...
public class AVL extends Structure {
    ...
    public Node Insert(int Number) {
        Node CurrRt = (Node) super.Insert(new Node(Number));
        Update();
        return CurrRt;
    }
    ...
    public Node Remove(int Number) {
        Node RmovNo = (Node) super.Remove(Number);
        Update();
        return RmovNo;
    }
    ...
    private void DepthsUpdate() {
        ...
        for (int i = Travel.size(); i > 0; i--) {
            DrawAVL.Node CurrNo = (DrawAVL.Node) Travel.get(i - 1);
            int LoopDL = CurrNo.L != null ? ((DrawAVL.Node) CurrNo.L).DepthN : 0;
            int LoopDR = CurrNo.R != null ? ((DrawAVL.Node) CurrNo.R).DepthN : 0;
